...
def appearance():
    return setting('theme.alt').lower()
...
def artPath():
    ...
    if theme in ['-', '']:
        return
    elif condVisibility('System.HasAddon(script.xprime.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.xprime.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def artwork():
    execute('RunPlugin(plugin://script.xprime.artwork)')
...
def moderator():
    try:
        ...
        if not plugin_name in white_list:
            xbmc.log('XPrime Moderator Blockage: %s (Contact me with this line if you feel its a error.)' % plugin_name, xbmc.LOGWARNING)
            sys.exit()
    except Exception as error:
        xbmc.log('XPrime Moderator Failure: %s' % error, xbmc.LOGDEBUG)
    ...
    try:
        ...
        if (art == None and theme in ['-', '']):
            if setting('show.artwork') == 'true':
                ...
                if not yes:
                    return
                installAddon('script.xprime.artwork')
        return
    except:
        return

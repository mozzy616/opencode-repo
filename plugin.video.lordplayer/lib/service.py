...
from lib.lordplayer.api import LordPlayer, STATUS_FINISHED, STATUS_SEEDING, STATUS_PAUSED
from lib.torrest_daemon import LordPlayerLibraryDaemon, LordPlayerExecutableDaemon, LordPlayerDaemonNotFoundError
...
class DaemonMonitor(xbmc.Monitor):
    ...
    def __init__(self):
        ...
        logging.info("Using '%s' as LordPlayer base directory", BASE_DIRECTORY)
        ...
        if os.path.exists(LIB_PATH) and not settings.get_force_LordPlayer():
            ...
            logging.info("Configuring LordPlayer executable (%s)", EXE_NAME)
            self._daemon = LordPlayerExecutableDaemon(
...
class DownloadProgress(xbmc.Monitor, threading.Thread):
    ...
    def onSettingsChanged(self):
        self._api = LordPlayer(settings.get_service_address(), settings.get_port(), settings.ssl_enabled())
...
def run():
    kodi.set_logger()
    settings.set_has_libLordPlayer(os.path.exists(LIB_PATH))
    handle_first_run()
    ...
    except LordPlayerDaemonNotFoundError as e:
